// ESP8266 BME280 + Neo-6M GPS Data Logger with Buffer Logic
// Install: "Adafruit BME280 Library" by Adafruit in Library Manager
...
#define RECORD_SIZE 24                // Size of each buffered record (increased for pressure)
#define MAX_RECORDS (MAX_BUFFER_SIZE / RECORD_SIZE)  // ~145 records
...
// Timing intervals - Updated for 30 second cycles
...
const unsigned long transmissionInterval = 30000;     // Changed to 30 seconds
...
} cachedData = {-999.0, -999.0, -999.0, false, 0};
...
// Buffered record structure (24 bytes) - Including pressure
struct BufferedRecord {
  ...
  float pressure;         // 4 bytes - Pressure in hPa
...
void setup() {
  ...
  Serial.println("\n\n=== ESP8266 BME280 + Neo-6M GPS Data Logger with Buffer ===");
  ...
  Serial.println("BME280 Wiring: D2(GPIO4)->SDA, D1(GPIO5)->SCL, 3.3V->VCC, GND->GND");
  Serial.println("Transmission: Every 30 seconds");
  Serial.println("============================================================\n");
  ...
  for (int attempt = 1; attempt <= 3; attempt++) {
    Serial.printf("BME280 Attempt %d/3...", attempt);
    
    if (bme.begin(0x76) || bme.begin(0x77)) { // Try both common I2C addresses
      delay(2000);
      
      float testTemp = bme.readTemperature();
      float testHumid = bme.readHumidity();
      float testPressure = bme.readPressure() / 100.0F; // Convert Pa to hPa
      
      if (!isnan(testTemp) && !isnan(testHumid) && !isnan(testPressure) && 
          testTemp > -40 && testTemp < 80 && testHumid >= 0 && testHumid <= 100 &&
          testPressure > 300 && testPressure < 1200) {
        Serial.printf(" ✅ SUCCESS - Temp: %.1f°C, Humidity: %.1f%%, Pressure: %.1f hPa\n", 
                      testTemp, testHumid, testPressure);
        bmeWorking = true;
        cachedData.temperature = testTemp;
        cachedData.humidity = testHumid;
        cachedData.pressure = testPressure;
  ...
  if (!bmeWorking) {
    Serial.println("❌ BME280 sensor failed! Check wiring and I2C address.");
  }
  ...
  setupCompleted = true;
  Serial.println("🚀 Setup completed! Starting main loop with 30-second transmission cycle...\n");
}
...
void loop() {
  ...
  // Transmit data every 30 seconds (online) or buffer data (offline)
...
void initializeBuffer() {
  ...
  bool bufferValid = (bufferHeader.bufferVersion == 1002 && 
  ...
  if (!bufferValid) {
    ...
    bufferHeader.bufferVersion = 1002; // Updated version for BME280
...
void bufferData() {
  ...
  Serial.printf("📦 BUFFERED: %.1f°C, %.1f%%, %.1f hPa at %s - Buffer: %d/%d records\n",
...
void transmitBufferedData() {
  ...
  for (int i = 0; i < recordsToProcess; i++) {
    ...
    String payload = createPayload(record.temperature, record.humidity, record.pressure,
                                 record.latitude, record.longitude, timestamp);
    
    if (sendDataToServer(payload)) {
      transmitted++;
      bufferHeader.totalTransmitted++;
      Serial.printf("✅ Transmitted buffered data: %.1f°C, %.1f%%, %.1f hPa at %s\n", 
                    record.temperature, record.humidity, record.pressure, timestamp.c_str());
      removeOldestRecord();
    } else {
      Serial.printf("❌ Failed to transmit buffered data: %.1f°C, %.1f%%, %.1f hPa\n", 
                    record.temperature, record.humidity, record.pressure);
...
void readSensorData() {
  ...
  float pressureValue = bme.readPressure() / 100.0F; // Convert Pa to hPa
  
  if (isnan(temperature) || isnan(humidityValue) || isnan(pressureValue) || 
      temperature < -40 || temperature > 80 || humidityValue < 0 || humidityValue > 100 ||
      pressureValue < 300 || pressureValue > 1200) {
    sensorFailures++;
    Serial.printf("⚠️  BME280 read failed (Failures: %d)\n", sensorFailures);
  } else {
    cachedData.temperature = temperature;
    cachedData.humidity = humidityValue;
    cachedData.pressure = pressureValue;
    cachedData.isValid = true;
    cachedData.lastReadTime = millis();
    
    Serial.printf("🌡️  Sensor: %.1f°C, %.1f%%, %.1f hPa\n", temperature, humidityValue, pressureValue);
  }
}
...
void transmitData() {
  ...
  Serial.println("📤 DATA TRANSMITTED (30s cycle)");
  Serial.printf("   📊 Temp: %.1f°C, Humidity: %.1f%%, Pressure: %.1f hPa\n", 
                cachedData.temperature, cachedData.humidity, cachedData.pressure);
...
void printSystemStatus() {
  ...
  if (cachedData.isValid) {
    Serial.printf(" (%.1f°C, %.1f%%, %.1f hPa)", cachedData.temperature, cachedData.humidity, cachedData.pressure);
  }
  Serial.println();
...
String formatTimestamp(unsigned long epochTime) {
  // Format timestamp without timezone suffix as requested
