// ESP8266 HTU21D + Neo-6M GPS Data Logger with Buffer Logic
// Install: "SparkFun HTU21D Humidity and Temperature Sensor Breakout" by SparkFun in Library Manager
// Or: "Adafruit HTU21DF Library" by Adafruit

#include <ESP8266WiFi.h>
#include <ESP8266HTTPClient.h>
#include <WiFiClient.h>
#include <SparkFunHTU21D.h>  // Changed from Adafruit_AHTX0.h
...
const char* secretKey = "lostdev-sensor3-1008200302082003";
...
HTU21D htu21d;  // Changed from Adafruit_AHTX0 aht;
...
void setup() {
  ...
  Serial.println("\n\n=== ESP8266 HTU21D + Neo-6M GPS Data Logger with Buffer ===");
  ...
  Serial.println("HTU21D Wiring: D2(GPIO4)->SDA, D1(GPIO5)->SCL, 3.3V->VCC, GND->GND");
  Serial.println("============================================================\n");
  ...
  Serial.println("Testing HTU21D sensor...");
  bool htuWorking = false;
  for (int attempt = 1; attempt <= 3; attempt++) {
    Serial.printf("HTU21D Attempt %d/3...", attempt);
    
    // HTU21D begin() returns void, so we just call it and test readings
    htu21d.begin();
    delay(2000);
    
    float testTemp = htu21d.readTemperature();
    float testHumid = htu21d.readHumidity();
    ...
    if (!isnan(testTemp) && !isnan(testHumid) && testTemp > -40 && testTemp < 80 && testHumid >= 0 && testHumid <= 100) {
      ...
      Serial.printf(" ❌ FAILED - Invalid readings (T:%.1f H:%.1f)\n", testTemp, testHumid);
    }
    delay(2000);
  }
  
  if (!htuWorking) {
    Serial.println("❌ HTU21D sensor failed! Check wiring and I2C address.");
    Serial.println("   Make sure SDA->D2(GPIO4), SCL->D1(GPIO5), 3.3V power");
  }
